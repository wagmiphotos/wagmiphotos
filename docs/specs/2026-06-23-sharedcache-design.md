...
5. **StorageService** (`core/storage.py`) — wraps Genblaze's `genblaze-s3` (`S3StorageBackend.for_backblaze` + `ObjectStorageSink`); **B2 is reached through Genblaze, not a separate boto3 client.** The sink downloads each generated asset, computes SHA-256, uploads to B2, rewrites the asset URL to a durable B2 URL, and uploads the manifest — all inside one `pipeline.run(sink=...)`. This service also reads bytes back (`backend.get(key)`) so the Processor can derive thumbnails/WebP, which it re-uploads via the same backend. *Depends on:* `genblaze-s3` + B2 creds (`B2_KEY_ID`/`B2_APP_KEY`).

6. **GenblazeAdapter** (`core/genblaze_adapter.py`) — the miss path. Builds `Pipeline(name, project_id).step(provider, model=..., prompt=..., modality=Modality.IMAGE, **params)` and `.arun(sink=storage, raise_on_failure=True, timeout=...)`; returns the resulting `Asset` (durable B2 `url`, `sha256`, `width`/`height`, `media_type`) and `result.manifest` (auto-emitted; `canonical_hash` = SHA-256). Provider is selected by instantiating the class (`DalleProvider`/`ImagenProvider`/`GMICloudImageProvider`) with a `model=` slug; default **`gpt-image-1`** (returns base64, persisted by the sink — avoids dall-e-3's ~1h URL expiry). *Depends on:* `genblaze-core` + one connector extra (`genblaze-openai`/`-google`/`-gmicloud`).
...
1. **Genblaze API — VERIFIED (2026-06-23).** Pipeline builder API confirmed (§6). It's a client lib — **no Genblaze key**; per-provider keys + B2 keys via env/constructor. B2 storage + provenance manifest handled by Genblaze's own `genblaze-s3` sink; manifest auto-emitted per run. **Caveats to handle in code:** it's **alpha** with version churn → **pin** `genblaze-core` + connector + `genblaze-s3`; pass `raise_on_failure=True` (silent-fail is the default until 0.4.0) and check `step.status`; use `gpt-image-1` (base64) not dall-e-3 (~1h URL expiry); set `Pipeline(preflight=False)` in tests to avoid a network catalog call; Genblaze ships **no pricing data** (`cost_usd=None`) → CostMeter keeps its own table (§10); always persist via the sink so `Asset.sha256` is populated (provenance issue #77: `verify()` passes on URL-only assets).
...
- **Week 1:** Scaffold repo (uv project with pinned `genblaze-core`/connector/`genblaze-s3`, FastAPI, Postgres+pgvector, B2 bucket); StorageService (Genblaze S3 backend) + CacheIndex with tests; smoke-test one Genblaze image gen → durable B2 URL + manifest.
