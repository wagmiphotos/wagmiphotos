...
def test_defaults_apply_when_env_absent():
    ...
    assert s.default_image_model == "gpt-image-2-generate"
    assert s.generation_size == "1024x1024"
...
def test_new_defaults(monkeypatch):
    for k in ("DEFAULT_PROVIDER", "IMAGE_PRICE_USD", "GENERATION_MIN_REQUESTS",
              "WORKER_INTERVAL_SECONDS", "WORKER_BATCH_SIZE", "WORKER_MAX_SPEND_USD"):
        monkeypatch.delenv(k, raising=False)
    from wagmiphotos.common.config import Settings
    s = Settings(_env_file=None)
    assert s.default_provider == "gmicloud"
    assert s.image_price_usd == 0.055
    assert s.generation_min_requests == 10   # only generate for repeatedly-requested prompts
...
def test_cf_and_floor_defaults(monkeypatch):
    ...
    assert s.floor_sim_max == 0.84 and s.floor_sim_min == 0.75
