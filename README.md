...
Semantic search uses **BGE** (`bge-base-en-v1.5`, 768-dim, text-to-text): the Worker embeds query prompts with Cloudflare **Workers AI** (`@cf/baai/bge-base-en-v1.5`), and the backfill embeds asset prompts/captions with a local `BAAI/bge-base-en-v1.5` — the same base model, so both live in one embedding space. The API exposes no matching knobs at all: the request body is `prompt`, `n`, `size`, `collection` — no model parameter, and no `cache_tolerance` / `generate_on_miss` (both removed 2026-07-10). The similarity floor is fixed server-side. `GET /v1/library`'s search (`?q=`) is likewise **semantic** (BGE, shard fan-out, floor `library_floor_sim` = 0.6 — a deliberately lower, separate floor from the generation one, split off 2026-07-14 so short one-word queries still return results against verbose PD12M captions) — if the semantic query errors, production degrades to a likes-ranked browse and marks the response `"degraded": true` (the results are popular images, not matches for `q`). The old plain-SQL `LIKE` fallback is now dev-only: it cannot use an index and read the whole table on every search.
